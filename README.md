...
- `lambda.tf`
- `apigateway.tf`
- `frontend_deployment.sh`<- Bash script for modify the `frontend/singletone.js` and upload the frontend to the S3.
...
### IMPORTANT NOTE
Do not manually modify `frontend/singletone.js` if you intend to use this script for deployment. Manual changes might cause the script to malfunction (due to the PLACEHOLDER_ strings needed to the script replace them with the terraform outputs). Have in count also once you used the script, your local copy of the file `frontend/singletone.js` will contain important information about your API Gateway (ID, region, stage).
...
#### Security
The current setup exposes a security risk, particularly with the `submitScore` function, which could be exploited to submit scores from outside the app (CloudFront CDN website) source. A potential solution could be restricting origin access to a static domain origin CDN and/or implementing rate timing limiting of calling the submitScore API endpoint, but this requires additional frontend and backend logic and was out of scope (i think?) for this project.
